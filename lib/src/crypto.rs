...
fn decrypt_aes_256_cbc(ciphertext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>> {
    ...
    if padding_len == 0 || padding_len > 16 {
        return Err(ExtensionError::InvalidPadding(alloc::format!(
    ...
    for byte in buffer.iter().skip(padding_start) {
        // XOR each padding byte with expected value
        // If all are correct, result will be 0
        padding_valid |= byte ^ (padding_len as u8);
    }
    ...
    (0..s.len())
        ...
        .map(|i| {
            u8::from_str_radix(&s[i..i + 2], 16).map_err(|_| {
                ExtensionError::HexDecode(alloc::format!("Invalid hex character at position {}", i))
            })
        })
        .collect()
}
